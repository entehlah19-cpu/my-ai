...
export async function POST(request: Request) {
  try {
    ...
    // Kunci API baru kamu yang berawalan AQ. (format resmi dari Google Studio)
    const GEMINI_API_KEY = 'AQ.Ab8RN6KIRamsKBkOOs1Sv9VKd2CvW5tNKOssmtwE00MxDe1';

    // Memanggil model gemini-2.5-flash yang kompatibel di sisi server Vercel
    ...
    // Tampilkan pesan kesalahan jika otentikasi Google bermasalah
    if (data.error) {
    ...
    return NextResponse.json({ text: 'Terjadi kegagalan koneksi internal pada server.' }, { status: 500 });
  }
}
